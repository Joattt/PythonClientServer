...
if __name__ == '__main__':
    ...
    client_logger.info(f'Запущен клиент с параметрами - '
                       f'сервер: {server_addr}, порт: {server_port}, пользователь: {client_name}.')
    database = ClientDatabase(client_name)
    try:
        transport = ClientTransport(server_port, server_addr, database, client_name)
    except Exception as e:
        print(e)
        exit(1)
    ...
    main_window.setWindowTitle(f'Сетевой чат - {client_name}')
    client_app.exec_()
